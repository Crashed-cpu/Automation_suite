...
const emailVideoBtn = document.getElementById('emailVideo');
...
// Email configuration (same as in photo capture app)
const EMAIL_CONFIG = {
    service: 'gmail',
    from: 'your-email@gmail.com', // Should match the one in .env
    subject: 'Video from Video Recorder App'
};
...
function setupEventListeners() {
    ...
    emailVideoBtn.addEventListener('click', sendEmail);
    
    // Disable buttons initially
    stopRecordingBtn.disabled = true;
    emailVideoBtn.disabled = true;
...
function showPreview(videoUrl) {
    // Store the video URL for emailing
    ...
    // Show download section and enable email button
    downloadSection.style.display = 'block';
    downloadLink.href = videoUrl;
    downloadLink.download = `recording-${new Date().toISOString().slice(0, 10)}.mp4`;
    emailVideoBtn.disabled = false;
}
...
async function sendEmail() {
    if (!recordedVideoUrl) {
        showStatus('No video to send', 'error');
        return;
    }
    ...
    try {
        ...
        const message = prompt('Add a message (optional):', 'Here is the video I recorded!') || 'Here is the video I recorded!';
        
        // Show sending status
        showStatus('Sending email... This may take a moment for larger videos.', 'info');
        ...
        formData.append('attachments', videoBlob, 'recording.mp4');
